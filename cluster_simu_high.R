...
test_mode <- tolower(Sys.getenv("CLUSTER_SIMU_TEST_MODE")) %in% c("1", "true", "yes")
output1_dir <- if (test_mode) "output1_high_test" else "output1_high"
output2_dir <- if (test_mode) "output2_high_test" else "output2_high"

dir.create(output1_dir, showWarnings = FALSE, recursive = TRUE)
dir.create(output2_dir, showWarnings = FALSE, recursive = TRUE)

alpha_grid <- seq(0, 1, 0.05)
chi_square_simu_time <- 10000

high_dim_covariate_names <- paste0("x", 1:10)
continuous_covariate_names <- paste0("x", 1:5)
binary_covariate_names <- paste0("x", 6:10)

mean_outcome_library <- c("SL.glm", "SL.randomForest")
mean_propensity_library <- c("SL.glm")
iv_z_library <- c("SL.glm", "SL.randomForest")
iv_d_library <- c("SL.glm", "SL.randomForest")
iv_y_library <- c("SL.glm", "SL.randomForest", "SL.gam")

if (test_mode) {
  chi_square_simu_time <- 100
  mean_outcome_library <- c("SL.glm")
  iv_z_library <- c("SL.glm")
  iv_d_library <- c("SL.glm")
  iv_y_library <- c("SL.glm")
}
...
if (test_mode) {
  example_1_j_map <- list(`120` = c(2))
  example_2_j_map <- list(`160` = c(2))
}
...
mixed_transformed_covariates <- function(
    covariates,
    J,
    basis_type,
    continuous_names = continuous_covariate_names,
    binary_names = binary_covariate_names,
    include_intercept = TRUE,
    intercept_value = 1 / sqrt(2)) {
  x_df <- as.data.frame(covariates)
  if (is.null(names(x_df))) {
    names(x_df) <- paste0("x", seq_len(ncol(x_df)))
  }

  basis <- NULL
  if (include_intercept) {
    basis <- matrix(intercept_value, nrow = nrow(x_df), ncol = 1)
    colnames(basis) <- ""
  }

  for (cov_name in names(x_df)) {
    x <- as.numeric(x_df[[cov_name]])
    if (cov_name %in% continuous_names) {
      if (basis_type == "legendre") {
        cov_basis <- legendre_orthonormal_matrix(x, J)
        cov_basis <- cov_basis[, -1, drop = FALSE]
        colnames(cov_basis) <- paste0(cov_name, "_legendre_", seq_len(J))
      } else if (basis_type == "fourier") {
        cov_basis <- fourier_orthonormal_matrix(x, J)
        colnames(cov_basis) <- paste0(cov_name, "_fourier_", colnames(cov_basis))
      } else {
        stop("Unknown basis_type: ", basis_type, call. = FALSE)
      }
      basis <- cbind(basis, cov_basis)
    } else if (cov_name %in% binary_names) {
      binary_basis <- matrix(x, ncol = 1)
      colnames(binary_basis) <- cov_name
      basis <- cbind(basis, binary_basis)
    } else {
      warning(
        "Treating covariate '", cov_name,
        "' as continuous because it was not listed as binary.",
        call. = FALSE
      )
      if (basis_type == "legendre") {
        cov_basis <- legendre_orthonormal_matrix(x, J)
...
make_transformed_covariates <- function(covariates, J_values, basis_type) {
  basis_list <- lapply(
    J_values,
    function(J) mixed_transformed_covariates(
...
test_conditional_independence_np_example_1_high_dim <- function(dataset) {
  dt_0 <- dataset[dataset$a == 0, , drop = FALSE]
  continuous_terms <- continuous_covariate_names
  binary_terms <- paste0("factor(", binary_covariate_names, ")")
  formula_text <- paste(
    "y ~ factor(s) +",
    paste(c(continuous_terms, binary_terms), collapse = " + ")
  )
  bw <- np::npregbw(
...
run_example_1_replicate <- function() {
  ...
  for (n_name in names(example_1_j_map)) {
    ...
    for (scenario_index in seq_along(alpha_list)) {
      ...
      dataset <- dgp_mean_exchangeability_high_dim(n = n, alpha = alpha)
      psedo_timing <- time_expression(
        estimate_mean_exchangeability_pseudo_outcome(
          dataset,
          V = 2,
          covariate_names = high_dim_covariate_names,
          outcome_library = mean_outcome_library,
          propensity_library = mean_propensity_library
        )
      )
      ...
      racine_timing <- time_expression(
        tryCatch(
          test_conditional_independence_np_example_1_high_dim(dataset),
          error = function(e) e
        )
      )
...
run_example_2_replicate <- function() {
  ...
  for (n_name in names(example_2_j_map)) {
    ...
    for (scenario_index in seq_along(beta_list)) {
      ...
      dataset <- dgp_iv_compatibility_high_dim(n = n, beta = beta)
      psedo_timing <- time_expression(
        estimate_iv_compatibility_pseudo_outcome(
          dataset,
          V = 2,
          covariate_names = high_dim_covariate_names,
          z_library = iv_z_library,
          d_library = iv_d_library,
          y_library = iv_y_library
        )
      )
      psedo_dat <- psedo_timing$value
      nuisance_elapsed_seconds <- psedo_timing$elapsed_seconds
...
write.csv(
  example_1_results,
  file = file.path(output1_dir, paste0("sim_", touse, ".csv")),
  row.names = FALSE
)

write.csv(
  example_2_results,
  file = file.path(output2_dir, paste0("sim_", touse, ".csv")),
  row.names = FALSE
)
